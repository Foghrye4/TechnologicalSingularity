...
public class SlopeBlockStateContainer extends BlockStateContainer {
	private IBlockState[] propertyValueArray = new IBlockState[12];
	
	 public SlopeBlockStateContainer(Block blockIn, PropertySlopeNormal variant) {
	    ...
	    protected StateImplementation createState(Block block, ImmutableMap<IProperty<?>, Comparable<?>> properties,
	    	...
	    	if (propertyValueArray == null)
	            propertyValueArray = new IBlockState[12];
	        return new SlopeNormalStateImplementation(block, properties, propertyValueArray);
	    }
}
